	...
	<div class='comprehensiveOrder'>
		...
		<div class="header" style="background: #F2F2F2;padding:10px 0px;">
			<div class=" center">
				...
				<div class="d-flex" style="width:400px ;"><el-input :disabled="useType=='copy'" style="border-top-right-radius:0px ;border-bottom-right-radius: 0px;"  v-model="search" placeholder="根据客户姓名、电话、车牌号进行搜索"></el-input><el-button type="primary" @click='searchOld'>搜索</el-button></div>
		...
		<div class="body">
			<div class="b1">
				...
				<el-form class="d-flex" style="flex-wrap: wrap;" label-position="right" label-width="120px" :model="owner">
				  <el-form-item label="车牌号">
						<label slot="label"><span style="color: red;">*</span>车牌号</label>
				    <el-input v-model="owner.plateNumber" :disabled="dis||useType=='copy'" @change='importCarnum'></el-input>
				  </el-form-item>
				  <el-form-item label="客户手机号">
						<label slot="label"><span style="color: red;">*</span>客户手机号</label>
				    <el-input v-model="owner.customerPhone" :disabled='dis' @change='inputTel'></el-input>
				  </el-form-item>
				  <el-form-item label="客户姓名">
				    <el-input v-model="owner.customerName" :disabled='dis'></el-input>
				  </el-form-item>
					<el-form-item label="本次里程">
					  <el-input v-model.number="owner.lastMileage" :disabled='dis'></el-input>
					</el-form-item>
					...
					<el-form-item label="下次保养日期">
						<el-date-picker style='width: 202px;' v-model="owner.nextKeepFitTime" type="date" placeholder="填写日期有助于提醒用户" :disabled='dis' value-format='yyyy-MM-dd'></el-date-picker>
					</el-form-item>
					<el-form-item label="车架号">
					  <el-input v-model="owner.vehicleIdNum" placeholder='请输入车架号' :disabled='dis'></el-input>
					</el-form-item>
					<el-form-item label="发动机号">
					  <el-input placeholder='请输入发动机号' v-model="owner.engineNum" :disabled='dis'></el-input>
					</el-form-item>
					...
					<el-form-item label="车身颜色">
					  <el-input v-model="owner.carColor" placeholder='请输入车身颜色' :disabled='dis' @input='ic'></el-input>
					</el-form-item>
					
					
					<el-form-item label="保险到期日期">
						<el-date-picker style='width: 202px;' v-model="owner.compulsoryInsureDeadline" type="date" placeholder="请选择保险到期日期" :disabled='dis' value-format='yyyy-MM-dd'></el-date-picker>
					</el-form-item>
					<el-form-item label="年检日期">
						<el-date-picker style='width: 202px;' v-model="owner.annualInspectDate" type="date" placeholder="请选择年检日期" :disabled='dis' value-format='yyyy-MM-dd'></el-date-picker>
					</el-form-item>
					<el-form-item label="预计交车时间">
						    <el-date-picker style='width: 202px;' v-model="owner.estimateSubmit" type="datetime" placeholder="请选择交车时间" :disabled='dis' value-format='yyyy-MM-dd HH:mm:ss'>
						    </el-date-picker>
					</el-form-item>
				...
				<div class="farewell">
					...
					<div class="d-flex" style="align-items: center;">
						...
						<div>
						<el-input v-model="owner.repairName" placeholder='请输入送修人姓名' :disabled='dis'></el-input>
						</div>
					</div>
					<div class="d-flex" style="align-items: center;">
						...
						<el-input v-model="owner.repairPhone" placeholder='请输入送修人手机号' @change='inputTel2' :disabled='dis'></el-input>
						</div>
					</div>
					</div>
				</div>
			...
			<div class="b2" style="font-size: 14px;background: #fff;border-radius:15px ;margin-top: 10px;padding-left: 36px;">
				<div class="d-flex" style="align-items: center;">
					...
					<el-button type='text' style="margin-right: 30px;margin-left: 45px;" :class="useVip==true?'c1':''" @click="useVip==true?openVipPop():''">可用会员卡{{vipNum}}张</el-button>
					<div style="color: #999;">来店消费统计：来店消费<span>{{expense}}</span>次,消费{{price}}元&nbsp;&nbsp;挂账：{{buyer}}元</div>
				</div>
			...
			<div class="b3" style="background: #fff;border-radius:15px ;margin-top: 10px;padding-top: 10px;padding-bottom: 10px;">
				...
				<el-table  :data="manOur" :header-cell-style="{color:'#333',fontSize:'14px',fontWeight:'bold',fontSize:'16px',backgroundColor:'#d7d7d7'}" style="width: 100%" border>
				  ...
				  <el-table-column align='center' prop="serviceTime" label="服务时长" width="180">
					...
					<template slot-scope="scope">
						<el-input-number :min="1" :max="scope.row.surplusCount!=null?scope.row.surplusCount:20" size="mini" v-model="scope.row.goodsCount" @change="changeManOurNum(scope.row.price,scope.row.goodsCount,scope.$index, manOur)"  label="描述文字" :disabled="dis == false ? scope.row.isGift == 1 : dis"></el-input-number>
					</template>						
					</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 *编辑综合开单
			 */
			setList(customerName,customerPhone,plateNumber,mileage,engineNum,vehicleIdNum,applyDate,carColor,modelName,seriesName,customerCode,employeeId,updateId,workOrderTime,isMyCard,remarks,estimateSubmit,brandName,nextKeepFitKm,nextKeepFitTime,repairName,repairPhone,annualInspectDate,compulsoryInsureDeadline,list,laborOrderGoodsList,totalMoney){
				this.$http.post('/laborSynthesize/addLaborSynthesize',{
			 ...
			 * 新增开单
			 */
			addList(type,customerName,customerPhone,plateNumber,mileage,engineNum,vehicleIdNum,applyDate,carColor,modelName,seriesName,customerCode,employeeId,updateId,workOrderTime,isMyCard,remarks,estimateSubmit,brandName,nextKeepFitKm,nextKeepFitTime,repairName,repairPhone,annualInspectDate,compulsoryInsureDeadline,list,laborOrderGoodsList,totalMoney){
				this.$http.post('/laborSynthesize/addLaborSynthesize',{
					merchantCode:this.merchantCode,
			...
			getOtherVipGoods(customerCode){
				this.$http.get('/laborSynthesize/getOtherVip',{
					merchantCode:this.merchantCode,
					customerCode,
				...
				.then(res=>{
					 console.log('%c请求可用会员卡项目弹窗信息结果','color:red;font-size:20px',res)
					 if(res.code == '10000'){
...
// =======================================================================================================================================	
			...
			sendNv(sendPnv){
				...
				// 	let list= [];
				// 	list = sendPnv.filter(item=>{
				// 			// return this.manOur.indexOf(item) !== -1
				// 			return this.Clist.indexOf(item) !== -1
				// 	});
				// 	// console.log(111)
				// 	// this.manOur=list
				// 	this.Clist=list
				// }else{
					// console.log(222)
					// this.manOur=this.manOur.concat(sendPnv)
					 this.Clist=sendPnv
				// }
				...
				console.log('%c显示工111时','color:blue;background:red',this.manOur)
				this.nvIsShow=false
			},
			// 关闭弹窗
			closeNv(nvIsShow){
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				...
				priceChange(arr){
					// let total
					// console.log('%c辩护奥迪额数组','color:#70ff57;font-size:20px;font-weight:bold',arr)
					// arr.map(item => {
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++			
			...
			confirm(){
				...
				if(this.useType=='set'){
					// 编辑
					// console.log('公里数',f)
					this.setList(f.customerName,f.customerPhone,f.plateNumber,f.lastMileage,f.engineNum,f.vehicleIdNum,f.applyDate,f.carColor,modelName,seriesName,this.customerCode,this.createId,'',f.workOrderTime,isMyCard,this.otherRemark,f.estimateSubmit,brandName,f.nextKeepFitKm,f.nextKeepFitTime,f.repairName,f.repairPhone,f.annualInspectDate,f.compulsoryInsureDeadline,this.subjoin,arr,this.allNums)
				}else{
					
				this.addList('2',f.customerName,f.customerPhone,f.plateNumber,f.lastMileage,f.engineNum,f.vehicleIdNum,f.applyDate,f.carColor,modelName,seriesName,this.customerCode,this.createId,'007',f.workOrderTime,isMyCard,this.otherRemark,f.estimateSubmit,brandName,f.nextKeepFitKm,f.nextKeepFitTime,f.repairName,f.repairPhone,f.annualInspectDate,f.compulsoryInsureDeadline,this.subjoin,arr,this.allNums)
				}
			},
			...
			save(){
				...
				if(this.useType=='set'){
					...
					//正常开单 
				this.addList('1',f.customerName,f.customerPhone,f.plateNumber,f.lastMileage,f.engineNum,f.vehicleIdNum,f.applyDate,f.carColor,modelName,seriesName,this.customerCode,this.createId,'',f.workOrderTime,isMyCard,this.otherRemark,f.estimateSubmit,brandName,f.nextKeepFitKm,f.nextKeepFitTime,f.repairName,f.repairPhone,f.annualInspectDate,f.compulsoryInsureDeadline,this.subjoin,arr,this.allNums)
				}
			},
			...
			.then(res=>{
				...
				var modelName=res.data.modelName
						...
						let that=this
							...
							this.owner.carColor=res.data.vehicleColor||''
								console.log('车颜色',res.data.vehicleColor)
							 this.owner.applyDate=res.data.registeredDate||''
							this.vehicleId=res.data.vehicleId
							this.customerCode=res.data.customerCode
							}else{
