...
<template>
	<div class="quickOrderBill">
		<div style="padding-left: 30px;" class="choose">
			已选
			<span style="color: #0000FF;">{{ List2.length }}</span>
			个项目
		</div>

		<div style="padding-left:30px;margin-top: 20px;">
			<el-table :header-cell-style="{ color: '#333', fontSize: '14px', fontWeight: 'bold', fontSize: '20px', backgroundColor: '#d7d7d7' }" :data="List2" style="width: 100%">
				...
				<el-table-column align="center" prop="emp" label="销售/施工人员" width="180">
					<template slot-scope="scope">
						<div style="border:1px dashed #3f3fff;">
							<div style="position:relative;top:-4px;left:70px"><i @click="dis == false ? setEmpAlert(scope.$index, List2) : ''" class="el-icon-edit-outline"></i></div>
				...
				<el-table-column align="center" prop="" label="操作" width="180">
					<template slot-scope="scope">
						<div><el-button @click.native.prevent="deleteRow(scope.$index, List2)" style="color:red" type="text" size="small" :disabled="dis">删除</el-button></div>
						<div v-if="scope.row.isGift == 0"><el-button @click="set(scope.$index, List2)" type="text" size="small" :disabled="dis">设为赠品</el-button></div>
					</template>
				</el-table-column>
			</el-table>
		</div>
	     ...
	     if(!this.$route.meta.isBack || this.isFirstEnter){
	         ...
	         // this.getData();
				 ...
				 if(this.useType=='set'||this.useType=='copy'||this.useType=='look'){
				 	...
				 	this.$route.query.laborOrderGoodsList.map(item => {
				 ...
				 this.List.map(item => {
					 this.$route.query.laborOrderGoodsList.map(items=> {
				 ...
				 this.List=this.$route.query.laborOrderGoodsList
					 let all=0
				 this.List.map(item => {
				 	all += item.subtotal;
				 });
...
//挂账
		...
		handleChange(e) {
			...
			// 计算小计
			this.List.map(item=>{
				item.subtotal=(Number(item.num)*Number(item.price)).toFixed(2)
			})
			// 计算总价
			this.List.forEach(function(val, index) {
				totalPrice += Number(val.subtotal); //累计总价
